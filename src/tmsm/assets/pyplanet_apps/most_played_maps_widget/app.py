"""Most Played Maps podium widget.

Maps with the highest play count, read from stats_tracker. Height-aware
row count.
"""
...
class MostPlayedMapsWidget(WidgetAppBase):
    name = "pyplanet.apps.tmsm.most_played_maps_widget"
    label = "most_played_maps_widget"

    WIDGET_KEY = "most_played_maps"
    WIDGET_NAME = "Most Played Maps"
    WIDGET_DESCRIPTION = "Maps with the highest play count."
    WIDGET_ICON = "map"
    WIDGET_TEMPLATE = "most_played_maps_widget/most_played_maps.xml"
    ...
    def _format_value(self, item: dict[str, Any]) -> str:
        return str(int(item.get("plays", 0) or 0))

    async def _query(self, tracker, limit: int) -> list[dict[str, Any]]:
        return await tracker.query_most_played_maps(limit)
