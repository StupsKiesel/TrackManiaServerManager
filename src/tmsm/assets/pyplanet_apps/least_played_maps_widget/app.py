"""Least Played Maps podium widget.

Current-playlist maps that have gone longest without being played (never-played
first), read from stats_tracker. Height-aware row count.
"""
...
class LeastPlayedMapsWidget(WidgetAppBase):
    name = "pyplanet.apps.tmsm.least_played_maps_widget"
    label = "least_played_maps_widget"

    WIDGET_KEY = "least_played_maps"
    WIDGET_NAME = "Least Played Maps"
    WIDGET_DESCRIPTION = "Playlist maps least recently played."
    WIDGET_ICON = "hourglass"
    WIDGET_TEMPLATE = "least_played_maps_widget/least_played_maps.xml"
    ...
    async def _query(self, tracker, limit: int) -> list[dict[str, Any]]:
        return await tracker.query_least_played_maps(limit)
    ...
    async def get_widget_data(self, login: str) -> dict[str, Any]:
        ...
        return {
            "title": self.TITLE,
            "rows": rows,
            "note": "" if rows else "no playlist maps",
        }
