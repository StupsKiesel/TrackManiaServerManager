...
Most active (non-spectator) time on the server, read from stats_tracker.
...
class MostPlaytimeWidget(WidgetAppBase):
    name = "pyplanet.apps.tmsm.most_playtime_widget"
    label = "most_playtime_widget"

    WIDGET_KEY = "most_playtime"
    WIDGET_NAME = "Most Playtime"
    WIDGET_DESCRIPTION = "Players with the most active playtime on the server."
    WIDGET_ICON = "clock"
    WIDGET_TEMPLATE = "most_playtime_widget/most_playtime.xml"
    ...
    async def _query(self, tracker, limit: int) -> list[dict[str, Any]]:
        return await tracker.query_most_playtime(limit)
